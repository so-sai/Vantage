...
pub enum IntentKind {
    ...
    Lens,
    /// Seal codebase structure (Forensic Finalization) → `seal_structure` tool
    Seal,
    /// Verify codebase against seal → `verify_structure` tool
    Verify,
}
...
impl IntentKind {
    ...
    pub fn mcp_tool_name(&self) -> &'static str {
        match self {
            ...
            IntentKind::Seal => "seal_structure",
            IntentKind::Verify => "verify_structure",
        }
    }
}
...
impl IntentBuilder {
    ...
    pub fn build(self) -> Result<CognitiveIntent, IntentValidationError> {
        let intent = CognitiveIntent {
            intent: self
                .intent
                .ok_or(IntentValidationError::MissingIntentKind)?,
            source: self.source,
...
pub enum IntentValidationError {
    #[error("Missing intent kind - must specify Lens, Seal, or Verify")]
    MissingIntentKind,
    ...
    #[error("Seal intent requires Workspace target, got File")]
    SealRequiresWorkspace,

    #[error("Verify intent requires Workspace target, got File")]
    VerifyRequiresWorkspace,
