use vantage_types::{Manifest, AuthorityLevel};
    ...
    match ctx {
        ExecutionContext::Production => {
            if manifest.authority.authority_level != AuthorityLevel::Privileged {
                return EnforcementDecision::Reject("F2_UNAUTHORIZED: Production requires Privileged authority.".to_string());
            }
        }
        ...
        ExecutionContext::Lab => {
            ...
            if manifest.authority.authority_level == AuthorityLevel::Sandbox {
                 return EnforcementDecision::Reject("F2_UNAUTHORIZED: Lab requires at least Trusted authority.".to_string());
            }
        }
    }
